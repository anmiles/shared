        ...
        $section.templates.template | % {
            ...
            $section.sites.site | % {
                ...
                $urls | ? { $_ } | % {
                    ...
                    "`nCreateIISRecord -name $name -url $url -directory $directory -local_ip $($template.ip) -http $http -https $https -persistent $true -hosts $hosts -hosts_section $hosts_section -port $($template.port)"
